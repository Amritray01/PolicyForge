...
const StatCard = ({ title, value, icon: Icon, color, accentColor, delay = 0 }) => {
  ...
  return (
    <div className="glow-card p-6 group" style={{ animationDelay: `${delay}ms` }}>
...
const STATUS_COLORS = { OPEN: '#3B82F6', IN_PROGRESS: '#F97316', RESOLVED: '#10B981', ESCALATED: '#EF4444' };
...
const AdminDashboard = () => {
  ...
  const getWellnessColor = (score) => {
    if (score <= 20) return '#10b91bff';
    if (score <= 40) return '#59b16fff';
      ...
      <div className="relative z-10">
        <nav className="glass sticky top-0 z-50 px-6 py-3 rounded-none border-b-0 shadow-2xl">
          <div className="max-w-7xl mx-auto flex justify-between items-center">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #3B82F6, #F97316)', boxShadow: '0 0 16px rgba(59,130,246,0.4)' }}>
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => loadDashboardData(true)} disabled={refreshing} className="p-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition">
                <RefreshCw size={16} className={refreshing ? 'animate-spin text-blue-400' : 'text-white/60'} />
              </button>
              <button onClick={() => navigate('/admin/add-student')} className="flex items-center gap-2 bg-white text-blue-700 px-4 py-1.5 rounded-xl font-bold text-sm hover:bg-blue-50 transition shadow-lg">
        ...
        <div className="max-w-7xl mx-auto px-6 pt-8 space-y-8">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard title="Total Students" value={stats?.totalStudents} icon={Users} color="text-white" accentColor="#3B82F6" delay={0} />
            <StatCard title="High Priority" value={(stats?.highStressCount || 0) + (stats?.criticalCount || 0)} icon={AlertCircle} color="text-red-400" accentColor="#EF4444" delay={60} />
            <StatCard title="Open Tickets" value={stats?.pendingSupport || openTickets} icon={MessageSquare} color="text-blue-400" accentColor="#3B82F6" delay={120} />
            <StatCard title="Stable / Excellent" value={(stats?.excellentCount || 0) + (stats?.stableCount || 0)} icon={Award} color="text-orange-400" accentColor="#F97316" delay={180} />
          </div>
          ...
          <div className="flex items-center gap-1 p-1 rounded-xl bg-white/5 border border-white/8 w-fit">
            {[
              ...
              <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${activeTab === tab.id ? 'bg-white text-blue-800 shadow-md' : 'text-white/50 hover:text-white/80'}`}>
                  ...
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={courseStats} barGap={4}>
                        ...
                        <Bar dataKey="avgWellnessScore" fill="#3B82F6" radius={[6, 6, 0, 0]} name="Avg Wellness Score" />
          ...
          {/* Directory Tab */}
          {activeTab === 'directory' && (
            <div className="glow-card p-6 animate-in">
              <div className="glow-content">
                <div className="flex flex-col md:flex-row gap-3 mb-6">
                  <div className="flex-1 relative glow-input">
                    <div className="glow-content">
                      <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-white/30 z-10" size={16} />
                      <input type="text" placeholder="Search by name or ID…" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
                        className="w-full pl-9 pr-4 py-2.5 text-sm bg-transparent outline-none text-white placeholder:text-white/25 focus:border-blue-500/50 transition relative z-10" />
                    </div>
                  </div>
              ...
              <div className="glow-content">
                ...
                {tickets.length > 0 ? (
                  <div className="space-y-3">
                    {tickets.map((ticket) => (
                      <div key={ticket.id} className="p-4 rounded-xl border border-white/8 bg-white/3 hover:bg-white/5 transition-all glow-input">
                        <div className="glow-content">
